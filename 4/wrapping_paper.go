...
func calc_ribbon(sorted_box_dims []int)(int){
	// The input to calc_ribbon is required to be sorted

	// Calculate the volume of the box
	volume := sorted_box_dims[0] * sorted_box_dims[1] * sorted_box_dims[2]
	// Calculate the perimeter of the smallest face  (This functions due to the sorted requirement)
	perimeter := 2 * sorted_box_dims[0] + 2 * sorted_box_dims[1]
	// Return the sum of the volume and perimeter
...
func main() {
	...
	for {
		...
		ribbon += calc_ribbon(box_dims)
		length += 1
	}
	fmt.Println("The area required for all these damn presents is", area)
	fmt.Println("The length of ribbon required for all these damn presents is", ribbon)
